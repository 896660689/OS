...
if [ ! -f /tmp/ad_script_up.sh ]; then
	...
	wget --no-check-certificate -t 15 -T 50 -O /tmp/ad_script_up.sh https://raw.githubusercontent.com/896660689/OS/master/ad_script.sh;chmod 775 /tmp/ad_script_up.sh
...
if [ -x "/etc/storage/dnsmasq.d/ad_update.sh" ]; then
	...
	# 准备翻墙 AD 文件
	wget --no-check-certificate -t 10 -T 30 -qO /tmp/tmp_ad_up https://raw.githubusercontent.com/896660689/OS/master/tmp_ad_up && chmod 755 /tmp/tmp_ad_up && . /tmp/tmp_ad_up
fi
sleep 3 && exit 0
